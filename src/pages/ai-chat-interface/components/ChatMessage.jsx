...
const ChatMessage = ({ message, offlineMode = false }) => {
  ...
  /* =====================================================
 🚗 CAR RENTAL OFFER CARD (LOCALRENT)
===================================================== */
if (message?.type === "offer" && message?.card?.type === "car_rental") {
  const card = message.card;

  const city = card.title?.replace("Închirieri auto în ", "") ?? "destinația ta";
  const providerName = card.provider ?? "Localrent";
  const brandColor = "#10B981"; // verde friendly, travel-safe
  ...
  return (
    <div className="flex justify-start mb-6">
      <div className="flex w-full max-w-full sm:max-w-[92%] items-start gap-3">
        {/* Avatar AI */}
        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center bg-gradient-to-br from-emerald-500 to-emerald-700 shadow flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <h3 className="mt-2 font-semibold text-lg text-gray-900">
                    🚗 Închiriere auto în {city}
                  </h3>

                  <p className="mt-1 text-sm text-gray-600">
                    Mașini locale, fără surprize, direct de la furnizori din zonă.
                  </p>
                </div>
              ...
              <div className="mt-4">
                ...
                >
                  ...
                  >
                    <Icon name="ExternalLink" size={16} />
                    {card.cta?.label ?? "Vezi mașini"}
                  </Button>
                </a>

                <div className="mt-2 text-[11px] text-gray-400">
                  Link afiliat • disponibilitatea și prețurile pot varia
                </div>
              </div>
            </div>
          </div>
